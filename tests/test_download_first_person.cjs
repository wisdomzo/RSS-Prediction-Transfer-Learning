...
test('first person walks on ground, respects obstacles and boundaries, restores camera',()=>{
 ...
 ids['first-person'].fire('click');assert.equal(api.active,true);assert.equal(controls.enabled,false);assert.equal(camera.position.y,.7);
 ...
 api.exit();assert.deepEqual(camera.position,original);assert.equal(camera.fov,32);assert.equal(controls.enabled,true);assert.equal(ids['walk-controls'].hidden,true);
});
